...
class Player {
	double ax=0,ay=0,bx=0,by=0,speed=10,lenby2=0;;
...
public class pong extends JComponent implements Runnable, KeyListener{
	...
	public void paint(Graphics g)
	{
		...
		g2D.draw(new Line2D.Double(-10, -frame.getHeight()/2, -10, frame.getHeight()/2));
		...
		if(start==false) {
			...
			if(check==0) {
				...
				if(res<0) {
					...
					p2.reset();
					start = false;
					System.out.println("Player1: "+scoreP1+", Player2: "+scoreP2);
				}
			}
		}
		//run();
